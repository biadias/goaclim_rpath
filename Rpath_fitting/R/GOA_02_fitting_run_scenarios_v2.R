...
for (scen_name in names(scenarios_to_run)) {
  ...
  fit_result <- run_rpath_optim(
    ...
    species_vec = p$species, vartype_vec = p$vartype,
...
# ---------------------------------------------------------------------------- #
mzero_groups_2     <- c("walleye_pollock_adult", 
                        "pacific_cod_adult")
...
for (scen_name in names(scenarios_to_run)) {
  ...
  current_scene <- scenarios_to_run[[scen_name]]
  starts <- c(scenarios_to_run[[scen_name]]$params$MzeroMort[mzero_groups_2], rep(0, length(p$species)-2))
...
# Save results
saveRDS(results_61M02par, file = "Rpath_fitting/GOA/GOA_fit_results_61M02par_v3.rds")
...
for (scen_name in names(scenarios_to_run)) {
  ...
  starts <- c(scenarios_to_run[[scen_name]]$params$MzeroMort[mzero_groups_3], rep(0, length(p$species)-3))
  current_scene <- scenarios_to_run[[scen_name]]
...
# Save results
saveRDS(results_59M03par, file = "Rpath_fitting/GOA/GOA_fit_results_59M03par_v3.rds")
...
for (scen_name in names(scenarios_to_run)) {
  ...
  starts <- c(scenarios_to_run[[scen_name]]$params$MzeroMort[mzero_groups_4], rep(0, length(p$species)-4))
...
# Save results
saveRDS(results_59M04par, file = "Rpath_fitting/GOA/GOA_fit_results_59M04par_v3.rds")
...
starts <- c(scenarios_to_run[[scen_name]]$params$MzeroMort[mzero_groups_5], rep(0, length(p$species)-5)) 
...
# Save results
saveRDS(results_57M05par, file = "Rpath_fitting/GOA/GOA_fit_results_57M05par_v3.rds")
...
write.csv(aicc_table, "Rpath_fitting/GOA/wgoa_data_rpath_fitting/GOA_Model_AIC_Ranking_20260423.csv", row.names = FALSE)
